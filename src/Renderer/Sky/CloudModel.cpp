#include <pch.h>
#include <Renderer/Sky/CloudModel.h>
#include <Utils/FileReader.h>

#include <FastNoiseLite.h>

FastNoiseLite GetPerlinNoiseGenerator(int seed = 0, float frequency = 0.1f)
{
   FastNoiseLite perlinNoise;
   perlinNoise.SetNoiseType(FastNoiseLite::NoiseType::NoiseType_Perlin);
   perlinNoise.SetSeed(seed);
   perlinNoise.SetFrequency(frequency);
   perlinNoise.SetFractalType(FastNoiseLite::FractalType::FractalType_FBm);
   perlinNoise.SetFractalOctaves(3);
   perlinNoise.SetFractalLacunarity(2.88f);
   perlinNoise.SetFractalGain(0.36f);
   perlinNoise.SetFractalWeightedStrength(0.0f);
   return perlinNoise;
}

FastNoiseLite GetWorleyNoiseGenerator(int seed = 0, float frequency = 0.1f)
{
   FastNoiseLite worleyNoise;
   worleyNoise.SetNoiseType(FastNoiseLite::NoiseType::NoiseType_Cellular);
   worleyNoise.SetSeed(seed);
   worleyNoise.SetFrequency(frequency);
   worleyNoise.SetFractalType(FastNoiseLite::FractalType::FractalType_Ridged);
   worleyNoise.SetFractalOctaves(3);
   worleyNoise.SetFractalLacunarity(2.68f);
   worleyNoise.SetFractalGain(0.3f);
   worleyNoise.SetFractalWeightedStrength(0.1f);
   worleyNoise.SetCellularDistanceFunction(FastNoiseLite::CellularDistanceFunction::CellularDistanceFunction_EuclideanSq);
   worleyNoise.SetCellularReturnType(FastNoiseLite::CellularReturnType::CellularReturnType_Distance);
   worleyNoise.SetCellularJitter(1.6f);
   worleyNoise.SetDomainWarpType(FastNoiseLite::DomainWarpType_OpenSimplex2);
   worleyNoise.SetDomainWarpAmp(1.0f);
   return worleyNoise;
}

CloudsModel::CloudsModel(Renderer &renderer)
{
   LoadWeatherMapTexture(renderer, "assets/weather.png");
   GenerateBaseLowFreqNoiseTexture(renderer);
   GenerateBaseHighFreqNoiseTexture(renderer);
}

CloudsModel::~CloudsModel()
{
   delete _baseLowFreqNoiseTexture;
   delete _weatherMapTexture;
}

void CloudsModel::LoadWeatherMapTexture(Renderer &renderer, const std::string &filePath)
{
   int width, height;
   _weatherMapTexture = FileReader::LoadTexture2D(renderer, filePath, &width, &height);
   _weatherMapTextureDimensions = {static_cast<uint32_t>(width), static_cast<uint32_t>(height)};
   if (!_weatherMapTexture)
   {
      std::cerr << "Failed to load weather map texture from file: " << filePath << std::endl;
      return;
   }
}

void CloudsModel::GenerateWeatherMapTexture(Renderer &)
{
}

void CloudsModel::GenerateBaseLowFreqNoiseTexture(Renderer &renderer)
{
   _baseLowFreqNoiseTextureDimensions = {128u, 128u, 128u};

   // Base cloud density
   // Generate a low frequency Perlin Worlye (Perlin - Worley) noise and
   // a low frequency Worley noise
   const FastNoiseLite perlinNoise = GetPerlinNoiseGenerator(0, 0.046f);
   const FastNoiseLite worleyNoise = GetWorleyNoiseGenerator(1, 0.012f);

   const FastNoiseLite layeredWorleyNoise0 = GetWorleyNoiseGenerator(2, 0.046f);
   const FastNoiseLite layeredWorleyNoise1 = GetWorleyNoiseGenerator(3, 0.13f);
   const FastNoiseLite layeredWorleyNoise2 = GetWorleyNoiseGenerator(4, 0.18f);

   // Generate the 3D noise texture
   std::vector<uint8_t> noiseData(_baseLowFreqNoiseTextureDimensions.x * _baseLowFreqNoiseTextureDimensions.y * _baseLowFreqNoiseTextureDimensions.z * 4);
   for (uint32_t z = 0; z < _baseLowFreqNoiseTextureDimensions.z; ++z)
   {
      for (uint32_t y = 0; y < _baseLowFreqNoiseTextureDimensions.y; ++y)
      {
         for (uint32_t x = 0; x < _baseLowFreqNoiseTextureDimensions.x; ++x)
         {
            float perlinWorley = 0.0; {
               float perlin = Math::Remap(perlinNoise.GetNoise(static_cast<float>(x), static_cast<float>(y), static_cast<float>(z)), -1.0f, 1.0f, 0.0f, 1.0f);
               float worley = Math::Remap(worleyNoise.GetNoise(static_cast<float>(x), static_cast<float>(y), static_cast<float>(z)), -1.0f, 1.0f, 0.0f, 1.0f);
               perlinWorley = Math::Clamp(perlin * (1.0f - worley), 0.0f, 1.0f);
            }
            float layeredWorley0 = Math::ClampRemap(layeredWorleyNoise0.GetNoise(static_cast<float>(x), static_cast<float>(y), static_cast<float>(z)),
                                                    -1.0f, 1.0f, 0.0f, 1.0f);
            float layeredWorley1 = Math::ClampRemap(layeredWorleyNoise1.GetNoise(static_cast<float>(x), static_cast<float>(y), static_cast<float>(z)),
                                                    -1.0f, 1.0f, 0.0f, 1.0f);
            float layeredWorley2 = Math::ClampRemap(layeredWorleyNoise2.GetNoise(static_cast<float>(x), static_cast<float>(y), static_cast<float>(z)),
                                                    -1.0f, 1.0f, 0.0f, 1.0f);

            uint32_t idx = (z * _baseLowFreqNoiseTextureDimensions.x * _baseLowFreqNoiseTextureDimensions.y + y * _baseLowFreqNoiseTextureDimensions.x + x) * 4;
            noiseData[idx + 0] = static_cast<uint8_t>(perlinWorley * 255.0f); // R channel
            noiseData[idx + 1] = static_cast<uint8_t>((1.0f - layeredWorley0) * 255.0f); // G channel
            noiseData[idx + 2] = static_cast<uint8_t>((1.0f - layeredWorley1) * 255.0f); // B channel
            noiseData[idx + 3] = static_cast<uint8_t>((1.0f - layeredWorley2) * 255.0f); // A channel
         }
      }
   }

   // Create the texture
   WGPUTextureDescriptor textureDesc = {};
   textureDesc.usage = WGPUTextureUsage_TextureBinding | WGPUTextureUsage_CopyDst;
   textureDesc.dimension = WGPUTextureDimension_3D;
   textureDesc.size.width = _baseLowFreqNoiseTextureDimensions.x;
   textureDesc.size.height = _baseLowFreqNoiseTextureDimensions.y;
   textureDesc.size.depthOrArrayLayers = _baseLowFreqNoiseTextureDimensions.z;
   textureDesc.format = WGPUTextureFormat_RGBA8Unorm;
   textureDesc.mipLevelCount = 1;
   textureDesc.sampleCount = 1;
   _baseLowFreqNoiseTexture = new Texture(renderer.GetDevice(), &textureDesc);
   if (!_baseLowFreqNoiseTexture->IsValid())
   {
      std::cerr << "Failed to create base noise texture." << std::endl;
      return;
   }

   // Upload data to the texture
   WGPUExtent3D copyExtent = {_baseLowFreqNoiseTextureDimensions.x, _baseLowFreqNoiseTextureDimensions.y, _baseLowFreqNoiseTextureDimensions.z};
   renderer.UploadTextureData(*_baseLowFreqNoiseTexture, noiseData.data(), noiseData.size() * sizeof(uint8_t), &copyExtent);
}

void CloudsModel::GenerateBaseHighFreqNoiseTexture(Renderer &renderer)
{
   _baseHighFreqNoiseTextureDimensions = {32u, 32u, 32u};

   // Base cloud density
   // Generate a high frequency worley noise at increasing frequencies
   const FastNoiseLite layeredWorleyNoise0 = GetWorleyNoiseGenerator(5, 0.18f);
   const FastNoiseLite layeredWorleyNoise1 = GetWorleyNoiseGenerator(6, 0.28f);
   const FastNoiseLite layeredWorleyNoise2 = GetWorleyNoiseGenerator(7, 0.38f);

   // Generate the 3D noise texture
   std::vector<uint8_t> noiseData(_baseHighFreqNoiseTextureDimensions.x * _baseHighFreqNoiseTextureDimensions.y * _baseHighFreqNoiseTextureDimensions.z * 4);
   for (uint32_t z = 0; z < _baseHighFreqNoiseTextureDimensions.z; ++z)
   {
      for (uint32_t y = 0; y < _baseHighFreqNoiseTextureDimensions.y; ++y)
      {
         for (uint32_t x = 0; x < _baseHighFreqNoiseTextureDimensions.x; ++x)
         {
            float layeredWorley0 = Math::ClampRemap(layeredWorleyNoise0.GetNoise(static_cast<float>(x), static_cast<float>(y), static_cast<float>(z)),
                                                    -1.0f, 1.0f, 0.0f, 1.0f);
            float layeredWorley1 = Math::ClampRemap(layeredWorleyNoise1.GetNoise(static_cast<float>(x), static_cast<float>(y), static_cast<float>(z)),
                                                    -1.0f, 1.0f, 0.0f, 1.0f);
            float layeredWorley2 = Math::ClampRemap(layeredWorleyNoise2.GetNoise(static_cast<float>(x), static_cast<float>(y), static_cast<float>(z)),
                                                    -1.0f, 1.0f, 0.0f, 1.0f);

            uint32_t idx = (z * _baseHighFreqNoiseTextureDimensions.x * _baseHighFreqNoiseTextureDimensions.y + y * _baseHighFreqNoiseTextureDimensions.x + x) * 4;
            noiseData[idx + 0] = static_cast<uint8_t>((1.0f - layeredWorley0) * 255.0f); // R channel
            noiseData[idx + 1] = static_cast<uint8_t>((1.0f - layeredWorley1) * 255.0f); // G channel
            noiseData[idx + 2] = static_cast<uint8_t>((1.0f - layeredWorley2) * 255.0f); // B channel
            noiseData[idx + 3] = 0; // A channel
         }
      }
   }

   // Create the texture
   WGPUTextureDescriptor textureDesc = {};
   textureDesc.usage = WGPUTextureUsage_TextureBinding | WGPUTextureUsage_CopyDst;
   textureDesc.dimension = WGPUTextureDimension_3D;
   textureDesc.size.width = _baseHighFreqNoiseTextureDimensions.x;
   textureDesc.size.height = _baseHighFreqNoiseTextureDimensions.y;
   textureDesc.size.depthOrArrayLayers = _baseHighFreqNoiseTextureDimensions.z;
   textureDesc.format = WGPUTextureFormat_RGBA8Unorm;
   textureDesc.mipLevelCount = 1;
   textureDesc.sampleCount = 1;
   _baseHighFreqNoiseTexture = new Texture(renderer.GetDevice(), &textureDesc);
   if (!_baseHighFreqNoiseTexture->IsValid())
   {
      std::cerr << "Failed to create base noise texture." << std::endl;
      return;
   }

   // Upload data to the texture
   WGPUExtent3D copyExtent = {_baseHighFreqNoiseTextureDimensions.x, _baseHighFreqNoiseTextureDimensions.y, _baseHighFreqNoiseTextureDimensions.z};
   renderer.UploadTextureData(*_baseHighFreqNoiseTexture, noiseData.data(), noiseData.size() * sizeof(uint8_t), &copyExtent);
}
